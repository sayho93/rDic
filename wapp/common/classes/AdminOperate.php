...
if (! class_exists("AdminOperate"))
{
	...
	class AdminOperate extends AdminBase
	{
		...
		function getListOfEndHistory()
		{
			...
			SELECT COUNT(*) as rn
			FROM
			tbl_room_history AS `R` ,
			tbl_end_chunggu AS `E`
			WHERE 
			`R`.`no`='{$no}'
			AND `E`.`room_fk`=`R`.`pno` 
			AND `E`.`status`='Y'
		...
		function getListOfEndHistoryForExcel()
		{
			...
			$sql="
			SELECT `R`.`name`, `E`.`no`, `R`.`entering_due_date`, `R`.`leaving_due_date`, `R`.`contractor_name`, `R`.`deposit`, `R`.`balance` AS Rbalance, `E`.`subtraction_total`, `E`.`balance` AS Ebalance, `E`.`is_paid`
			FROM
			tbl_room_history AS `R` ,
			tbl_end_chunggu AS `E`
			WHERE 
		...
		function getInfoOfEnd()
		{
			...
			if(empty($key)){
				$sql="
					SELECT R.user_fk, B.name AS Bname, R.name AS Rname, R.contractor_name, YEAR(R.leaving_due_date) AS leaving_y, MONTH(R.leaving_due_date) AS leaving_m, DAYOFMONTH(R.leaving_due_date) AS leaving_d, R.balance
					FROM tbl_room AS R
				...
				$sql="
					SELECT  
						R.user_fk, R.name AS Rname, R.contractor_name, YEAR(R.leaving_due_date) AS leaving_y, MONTH(R.leaving_due_date) AS leaving_m, 
						DAYOFMONTH(R.leaving_due_date) AS leaving_d, E.month, E.subtraction_total, E.balance AS Ebalance, E.daily_fee, E.utility_fee, 
						E.maintenance_fee, E.leaving_fee, E.disposal_fee, E.damage_fee_1, E.damage_fee_2, E.damage_fee_3, E.damage_fee_4, E.cancelation_fee, 
						E.electricity_usage_lm, E.gas_usage_lm, E.water_usage_lm, E.electricity_usage, E.gas_usage, E.water_usage, R.balance
					FROM tbl_end_chunggu AS E 
					JOIN tbl_room_history AS R ON E.room_fk=R.pno
					WHERE E.no='{$key}'
					LIMIT 1
				";
				$result=$this->getRow($sql);
		...
		function getInfoOfRoomName()
		{
			$no=$this->req["no"];
			
			$sql="
				SELECT R.name, B.name AS Bname
				FROM tbl_room R 
		...
		function getInfoOfRent()
		{
			...
			if(empty($key)){
				...
				$sql="
					SELECT RC.*, R.name, B.name AS Bname
					FROM tbl_rent_chunggu RC
					JOIN tbl_room R ON RC.rNo=R.no
					JOIN tbl_building B ON R.building_fk=B.no
					WHERE RC.no='{$key}'
					LIMIT 1
				";
		...
		function getInfoOfLMRent()
		{
			...
			if(!empty($key)){
				$sql="
				SELECT MONTH(month) AS M, YEAR(month) AS Y, rNo
				FROM tbl_rent_chunggu
				WHERE no='{$key}'
				LIMIT 1
				";
				...
				$sql="
				SELECT no, sum_after_payment, is_paid
				FROM tbl_rent_chunggu RC
				WHERE rNo='{$room_fk}' AND RC.NO < '{$key}'
				ORDER BY no DESC
				LIMIT 1
				";
		...
		function getInfoOfBill()
		{
			...
			if(empty($key)){
				...
				$sql="
					SELECT BC.*, B.name AS Bname, R.name 
					FROM tbl_bill_chunggu BC
					JOIN tbl_room R ON BC.rNo=R.no
					JOIN tbl_building B ON R.building_fk = B.no
					WHERE BC.no='{$key}'
					LIMIT 1
				";
		...
		function saveEnd()
		{
			...
			$electricity_usage_lm=$this->req["electricity_usage_lm"];
			$gas_usage_lm=$this->req["gas_usage_lm"];
			$water_usage_lm=$this->req["water_usage_lm"];
			...
			if(empty($key)){
				$sql="
				INSERT INTO
				tbl_end_chunggu(user_fk, room_fk, month, subtraction_total, balance, daily_fee, utility_fee, maintenance_fee, leaving_fee, disposal_fee, damage_fee_1, damage_fee_2, damage_fee_3,
				damage_fee_4, cancelation_fee, electricity_usage_lm, gas_usage_lm, water_usage_lm, electricity_usage, gas_usage, water_usage, is_paid,status, leaving_due_date)
				VALUES
				('{$user_fk}', '{$pno}', '{$month}', '{$subtraction_total}', '{$balance}', '{$daily_fee}', '{$utility_fee}', '{$maintenance_fee}', '{$leaving_fee}', '{$disposal_fee}', '{$damage_fee_1}',
				 '{$damage_fee_2}', '{$damage_fee_3}', '{$damage_fee_4}', '{$cancelation_fee}', '{$electricity_usage_lm}', '{$gas_usage_lm}', '{$water_usage_lm}', 
				 '{$electricity_usage}', '{$gas_usage}', '{$water_usage}', 2, 'Y', '{$leaving_due_date}')
				";
				...
				$sql="
					...
					SET
						...
						`electricity_usage_lm`='{$electricity_usage_lm}',
						`gas_usage_lm`='{$gas_usage_lm}',
						`water_usage_lm`='{$water_usage_lm}',
		...
		function saveBill()
		{
			...
			if(empty($key)){
				$sql="
				INSERT INTO
				tbl_bill_chunggu(rNo, room_fk, month, electricity_check_date, gas_check_date, water_check_date, community_electricity_check_date, community_water_check_date,
				...
				gas_usage, water_usage, electricity_usage_lm, gas_usage_lm, water_usage_lm, is_paid, status, current_month_charge, billing_date)
				VALUES
				('{$rNo}', '{$pno}', '{$month}', '{$electricity_check_date}', '{$gas_check_date}', '{$water_check_date}', '{$community_electricity_check_date}', '{$community_water_check_date}',
				...
				'{$electricity_usage}', '{$gas_usage}', '{$water_usage}', '{$electricity_usage_lm}', '{$gas_usage_lm}', '{$water_usage_lm}', 2, 'Y', '{$current_month_charge}', '{$billing_date}')
		...
		function saveRent()
		{
			...
			if($key==""){
				$sql="
				INSERT INTO
				tbl_rent_chunggu(rNo, room_fk, month, maintenance_fee, internet, cable, rent_1, rent_2, etc, monthly_rent, current_month_late_fee, sum_after_payment, is_paid, status, current_month_charge, monthly_rent_date, payment_owner)
				VALUES
				('{$no}', '{$pno}', '{$month}', '{$maintenance_fee}', '{$internet}', '{$cable}', '{$rent_1}', '{$rent_2}', '{$etc}', '{$monthly_rent}', '{$current_month_late_fee}', '{$sum_after_payment}', 2, 'Y', '{$current_month_charge}', '{$monthly_rent_date}', '{$payment_owner}')			
				";
			
				$this->update($sql);
					
				return $this->makeResultJson(1, "등록되었습니다.".$rNo);
			}
			else{
				$sql="
